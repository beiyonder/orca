...
6. **Architecture decision register** — the atlas contains 42 versioned decisions/hypotheses with evidence and reversal conditions.
7. **Scope control** — 52 S1 deferrals were reviewed at `G1-RSCH`; none was silently promoted.

These are research/design artifacts plus bounded implementation evidence—not proof that the complete substrate works. Phase 3, `G4-AGNT`, and `G5-KNOW` pass. `P6-DISC-01` freezes Pagila v3.1.0. `P6-DISC-02` through `P6-DISC-07` now add versioned read-only adapter/access/request/observation contracts, exact cross-record authority, PostgreSQL mutation-proof bounded execution, complete/partial/denied/unavailable system and schema inventory, digest-only profiles, checksum-bound code extraction, and catalog/static/query-log/runtime lineage distinctions. Pagila reconstruction proves 30 relations, 58 relation constraints, 56 indexes, 10 routines, 15 triggers, three types, 13 sequences, exact actor profiles, 36 foreign-key and seven partition edges plus view/trigger dependencies. Migration 010 binds 64 schemas at fingerprint `374e03e9…82ec42`; 30 unit files / 167 tests and 16 PostgreSQL files / 48 tests pass. `P6-DISC-08` is current: CDC behavior must come from explicit versioned traces, not schema inference.
