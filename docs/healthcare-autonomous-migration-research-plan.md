...
6. **Architecture decision register** — the atlas contains 47 versioned decisions/hypotheses with evidence and reversal conditions.
7. **Scope control** — 52 S1 deferrals were reviewed at `G1-RSCH`; none was silently promoted.

These are research/design artifacts plus bounded implementation evidence—not proof that the complete substrate works. Phase 3, `G4-AGNT`, `G5-KNOW`, and `G6-DISC` pass. Phase 6 supplies frozen discovery and CDC truth. `P7-EVAL-01` through `03` add immutable independent contracts, durable coordination and the first executable structural oracle. `P7-EVAL-04` through `06` now add exact CDC count/key/delete/order/watermark/replay/disposition evaluation, two clean private TypeScript builds with compiler/provenance pins, and a ten-group held-out semantic scorer where false accepts fail and abstention/disagreement remain inconclusive. Registry 83, migrations 15, fingerprint `8681ec82…0864993a`; 38 unit files / 207 tests and 22 PostgreSQL files / 57 tests pass. `P7-EVAL-07` is current: only product-owned state may accept, reject or quarantine a subject.
