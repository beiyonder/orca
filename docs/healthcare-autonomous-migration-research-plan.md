...
6. **Architecture decision register** — the atlas contains 40 versioned decisions/hypotheses with evidence and reversal conditions.
7. **Scope control** — 52 S1 deferrals were reviewed at `G1-RSCH`; none was silently promoted.

These are research/design artifacts plus bounded implementation evidence—not proof that the complete substrate works. Phase 3, `G4-AGNT`, and `G5-KNOW` pass. Phase 5 provides governed corpus/retrieval/context/memory/skill capability. `P6-DISC-01` now freezes Pagila `pagila-v3.1.0` at `fef96757…ddf90e` with exact permissive license/schema/data bytes, PostgreSQL 16.15 UTF8/C, fixture digest `c22e7c17…f4fe025d`, and a real-server-qualified catalog/row-count oracle. It was selected over WideWorldImporters, Oracle samples, Synthea, and Debezium examples because it supplies relationships, partitions, routines, triggers, views, constraints, full-text behavior, and nontrivial data through the existing cross-platform PostgreSQL test boundary. `P6-DISC-02` is current: define exact read-only capability, permission, evidence, limit, error, and recovery contracts before adapter execution.
