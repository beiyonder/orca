...
6. **Architecture decision register** — the atlas contains 30 versioned decisions/hypotheses with evidence and reversal conditions.
7. **Scope control** — 52 S1 deferrals were reviewed at `G1-RSCH`; none was silently promoted.

These are research/design artifacts, not proof that the complete substrate works. Phase 2 supplies the reproducible lab; `P3-KERN-01/02` supply strict V1 contracts and a convergent PostgreSQL 16 schema. `P3-KERN-03` through `P3-KERN-06` now prove command identity, expected-version append, atomic event/projection/outbox state, leased delivery, and inbox deduplication. `P3-KERN-07` is current: validate immutable plan DAGs before task execution.
