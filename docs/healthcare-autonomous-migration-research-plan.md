...
6. **Architecture decision register** — the atlas contains 39 versioned decisions/hypotheses with evidence and reversal conditions.
7. **Scope control** — 52 S1 deferrals were reviewed at `G1-RSCH`; none was silently promoted.

These are research/design artifacts plus bounded implementation evidence—not proof that the complete substrate works. Phase 3, `G4-AGNT`, and `G5-KNOW` pass. Phase 5 now provides immutable governed corpus identity, complete provenance, authorization-before-ranking, structured/BM25/optional-semantic/bounded-graph retrieval, reproducible cited/redacted context, five-class quarantined memory, exact scope/use/invalidation history, and typed compatible skill lifecycle. Sealed `EXP-06` covers 20/20 known answers with 20/20 citations and zero denied use; sealed `EXP-07` improves 10/20 to 20/20 with zero poisoned, cross-tenant, or post-invalidation recall. `P6-DISC-01` is current: freeze the first licensed source fixture and exact version before adapter design.
