...
6. **Architecture decision register** — the atlas contains 43 versioned decisions/hypotheses with evidence and reversal conditions.
7. **Scope control** — 52 S1 deferrals were reviewed at `G1-RSCH`; none was silently promoted.

These are research/design artifacts plus bounded implementation evidence—not proof that the complete substrate works. Phase 3, `G4-AGNT`, `G5-KNOW`, and `G6-DISC` pass. Phase 6 now spans frozen licensed source and qualification fixtures, immutable read-only adapter authority, database-enforced observation limits, exact inventory/profile/code/lineage, explicit CDC replay, cited claim comparison, deterministic gap/probe reasoning, versioned target capability and a proposal-only five-task/22-mapping migration design. Sealed `EXP-02` finds 8/8 material contradictions with 10/10 citations and zero false promotion; `EXP-03` finds 9/10 planted items with zero decoys and 2/2 denials; `EXP-04` disposes 10/10 events to exact final state with zero gaps. Registry 71, migrations 11, fingerprint `7acfcb43…8769156`; 33 unit files / 176 tests and 18 PostgreSQL files / 53 tests pass. `P7-EVAL-01` is current: independent versioned evaluation must become the acceptance boundary before correction or learned capability promotion.
