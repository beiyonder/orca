...
describe('PostgreSQL specialized evaluator records', () => {
  it('persists all reports and rejects mutation', async () => {
    ...
    pools.push(pool)
    const records = Object.entries(SPECIALIZED_EVALUATOR_CONTRACT_SAMPLES).map(
      ([schemaName, payload]) => ({
        tenantId: payload.tenantId,
        recordId: payload.id,
        missionId: 'missionId' in payload ? payload.missionId : null,
        schemaName,
